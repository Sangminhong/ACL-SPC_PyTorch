...
class get_edge_feature(nn.Module):
    ...
    def forward(self,point_cloud):
        ...
        #point_cloud_neighbors=grouping_operation(point_cloud,idx.int())
...
class Generator(nn.Module):
    def __init__(self,params=None, device=torch.device("cuda:4")):
        ...
        self.up_projection_unit=up_projection_unit(device=self.device)
    ...
    def forward(self,input):
        ...
        x = torch.linspace(-1, 1, steps=16, device=features.device)
        y = torch.linspace(-1, 1, steps=16, device=features.device)
        z = torch.linspace(-1, 1, steps=16, device=features.device)
...
class attention_unit(nn.Module):
    def __init__(self,in_channels=130, device=None):
        ...
        #self.gamma=nn.Parameter(torch.tensor(torch.zeros([1]))).to(device)
...
class up_block(nn.Module):
    def __init__(self,up_ratio=2,in_channels=130, device=None):
        ...
        #self.grid=torch.tensor(self.gen_grid(up_ratio)).to(device)
        self.attention_unit=attention_unit(in_channels=in_channels, device=device)
    def forward(self,inputs):
        net=inputs #b,128,n
        device_num = net.get_device()
        ...
        device_num = torch.device(net.get_device())
        grid = grid.to(device_num)
...
class down_block(nn.Module):
    def __init__(self,up_ratio=2,in_channels=128):
...
class up_projection_unit(nn.Module):
    def __init__(self,up_ratio=2, device = None):
        super(up_projection_unit,self).__init__()
        self.conv1=nn.Sequential(
            Conv1d(in_channels=648,out_channels=128,kernel_size=1),
            nn.ReLU()
        )
        self.up_block1=up_block(up_ratio=2,in_channels=128+2, device = device)
        self.up_block2=up_block(up_ratio=2,in_channels=128+2, device = device)
        self.down_block=down_block(up_ratio=2,in_channels=128)
    def forward(self,input):
...
if __name__=="__main__":
    ...
    point_cloud=torch.rand(4,3,1000).cuda()
