...
class GRNet(torch.nn.Module):
    def __init__(self):
        ...
        self.point_sampling2 = RandomPointSampling(n_points=2048)
    ...
    def forward(self, data):
        partial_cloud = self.fc0(data).transpose(1,2)              # torch.Size([batch_size, 3096, 3])
        ...
        point_offset = self.fc14(point_features).view(-1, 8192, 3)
        # print(point_features.size())    # torch.Size([batch_size, 16384, 3])
        dense_cloud = sparse_cloud.unsqueeze(dim=2).repeat(1, 1, 4, 1).view(-1, 8192, 3) + point_offset
        # print(dense_cloud.size())       # torch.Size([batch_size, 16384, 3])

        dense_cloud = dense_cloud.transpose(1,2)
        return dense_cloud