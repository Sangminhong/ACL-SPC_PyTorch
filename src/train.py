...
import sys
import os
from data.data import NetDataset, PCNDataset 
sys.path.append(os.path.dirname(os.path.abspath(os.path.dirname(__file__))))
from models.polynet import PolyNet
from models.pcn import PCN
from models.polynet_enc_pcn_dec import PolyPCN
from models.pcnenc_polydec import PCN_Conv
...
class trainer(object):
    ...
    def train_epoch(self, epoch):
        ...
        for iter, data in enumerate(tqdm(self.train_loader)):
            ...
            if iter%10==0:              
                output_xyz(torch.transpose(out_final, 1,2)[0], '../outputs/'+ str(self.experiment_id)+ '/train/cpc_'+str(iter)+'.ply')
        ...
        print(f'Epoch {epoch+1}: Train_set_Chamfer {torch.mean(trainset_eval)}')

        if epoch%10==0:
           test_chamf = self.test_epoch(epoch)
           self._snapshot(epoch)
           return np.mean(loss_buf), test_chamf        
    ...
    def test_epoch(self, epoch):
        ...
        with torch.no_grad():
            ...
            print('test time: '+str(test_start.elapsed_time(test_end)/sample_size)) 
        return Dist1+Dist2
    ...
    def run(self,args):
        ...
        for epoch in tqdm(range(epoch_init, self.epochs)):  
            ...
            start_epoch.record()
            if epoch%10 ==0:
                loss, test_loss = self.train_epoch(epoch)
            else:    
            ...
            print('epoch_time: '+str(epoch_time)+'seconds')
            print('total_time for '+str(epoch)+'s: '+str(total_time)+'seconds')
